...
\begin{itemize}
    ...
    \item Uscite:
    \begin{itemize}
        \item \textbf{DONE} (1 bit): indica la fine dell'elaborazione.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    ...
    \item Quando \texttt{RESET} torna a \texttt{0}, il modulo attende che quando il \texttt{CLOCK} sale ad $1$ che \texttt{START} sia pari a $1$.
    \item Legge i dati da memoria, esegue il filtraggio e scrive i risultati.
    \item Alla fine dell'elaborazione, alza \texttt{DONE=1}.
    \item Il modulo attende che \texttt{START} torni a \texttt{0} prima di un nuovo ciclo.
    \item Infine \texttt{DONE=0} per prepararsi al prossimo ciclo.
\end{enumerate}

\noindent Nota: Prima del primo START=1, il modulo deve aver ricevuto RESET=1. Dopo la prima



\section{Sezione Due}
La seconda sezione della relazione, dove si possono presentare ulteriori analisi o risultati. Potremmo anche includere formule matematiche:
$$ E=mc^2 $$
Oppure inserire un'immagine (assicurati di avere un file `immagine.png` nella stessa cartella del file .tex):
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-image-a} % Sostituisci con il nome della tua immagine
    \caption{Una didascalia per l'immagine.}
    \label{fig:immagine}
\end{figure}

\section{Conclusioni}
In questa sezione riassumiamo i punti chiave della relazione e presentiamo le conclusioni. Si possono anche suggerire lavori futuri o ulteriori ricerche.
